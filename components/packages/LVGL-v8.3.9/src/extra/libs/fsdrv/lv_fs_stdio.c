...
static void * fs_open(lv_fs_drv_t * drv, const char * path, lv_fs_mode_t mode)
{
    ...
    fd = open(buf, flags, 0);
    if(fd < 0 && path[0] == '/') {
        fd = open(path + 1, flags, 0);
    }
    if(fd < 0 && path[0] == '/' && strncmp(path, "/assets/", 8) == 0) {
        lv_snprintf(buf, sizeof(buf), "/sdcard%s", path);
        fd = open(buf, flags, 0);
    }
    ...
    handle = lv_mem_alloc(sizeof(file_handle_t));
    if(handle == NULL) {
        close(fd);
        return NULL;
    }
    ...
    fp = fopen(buf, flags);
    if(fp == NULL && path[0] == '/') {
        fp = fopen(path + 1, flags);
    }
    if(fp == NULL && path[0] == '/' && strncmp(path, "/assets/", 8) == 0) {
        lv_snprintf(buf, sizeof(buf), "/sdcard%s", path);
        fp = fopen(buf, flags);
    }
...
#ifndef WIN32
    file_handle_t * handle = (file_handle_t *)file_p;
    int read_len = read(handle->fd, buf, btr);
    if(read_len < 0) {
        if(br != NULL) *br = 0;
        return LV_FS_RES_UNKNOWN;
    }
    if(br != NULL) *br = (uint32_t)read_len;
#else
    uint32_t read_len = fread(buf, 1, btr, file_p);
    if(br != NULL) *br = read_len;
#endif
    return LV_FS_RES_OK;
}
...
#ifndef WIN32
    file_handle_t * handle = (file_handle_t *)file_p;
    int write_len = write(handle->fd, buf, btw);
    if(write_len < 0) {
        if(bw != NULL) *bw = 0;
        return LV_FS_RES_UNKNOWN;
    }
    if(bw != NULL) *bw = (uint32_t)write_len;
#else
...
static lv_fs_res_t fs_seek(lv_fs_drv_t * drv, void * file_p, uint32_t pos, lv_fs_whence_t whence)
{
    ...
    if(whence == LV_FS_SEEK_CUR) native_whence = SEEK_CUR;
    else if(whence == LV_FS_SEEK_END) native_whence = SEEK_END;
    return lseek(handle->fd, pos, native_whence) < 0 ? LV_FS_RES_UNKNOWN : LV_FS_RES_OK;
#else
...
#ifndef WIN32
    file_handle_t * handle = (file_handle_t *)file_p;
    int pos = lseek(handle->fd, 0, SEEK_CUR);
    if(pos < 0) {
        *pos_p = 0;
        return LV_FS_RES_UNKNOWN;
    }
